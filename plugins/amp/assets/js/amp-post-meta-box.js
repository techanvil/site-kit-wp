...
/*!********************************************************!*\
  !*** ./assets/src/classic-editor/amp-post-meta-box.js ***!
  \********************************************************/
/* global jQuery */
...
window.ampPostMetaBox = function ($) {
  ...
  const component = {
    ...
    data: {
      canonical: false,
      // Overridden by amp_is_canonical().
      previewLink: '',
      enabled: true,
      // Overridden by amp_is_post_supported( $post ).
      canSupport: true,
      // Overridden by count( AMP_Post_Type_Support::get_support_errors( $post ) ) === 0.
      statusInputName: '',
      l10n: {
        ampPreviewBtnLabel: ''
      }
  ...
  component.addPreviewButton = function addPreviewButton() {
    ...
    previewBtn.clone().insertAfter(previewBtn).prop({
      href: component.data.previewLink,
      id: component.ampPreviewBtnSelector.replace('#', '')
    }).text(component.data.l10n.ampPreviewBtnLabel).parent().addClass('has-amp-preview');
  };
  /**
  ...
  component.onAmpPreviewButtonClick = function onAmpPreviewButtonClick() {
    ...
    const $input = $('<input>').prop({
      ...
      name: 'amp-preview',
      value: 'do-preview'
    }).insertAfter(component.ampPreviewBtnSelector); // Trigger Core preview button and remove AMP flag.
  ...
  component.toggleAmpStatus = function toggleAmpStatus($target) {
    ...
    let status = $container.data('amp-status'); // Don't modify status on cancel button click.

    if (!$target.hasClass('button-cancel')) {
      status = component.statusRadioInputs.filter(':checked').val();
    }

    const $checked = $('#amp-status-' + status); // Toggle elements.
    ...
    $container.slideToggle(component.toggleSpeed); // Update status.
...
/******/ })()
;
//# sourceMappingURL=amp-post-meta-box.js.map