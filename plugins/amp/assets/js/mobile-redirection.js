...
(function (_ref) {
  ...
  const regExpRegExp = new RegExp(regexRegex); // A user agent may be expressed as a RegExp string serialization in the the form of `/pattern/[i]*`. If a user
  ...
  __webpack_require__.g.addEventListener('DOMContentLoaded', () => {
  ...
  const amphtmlUrlObject = new URL(ampUrl); // Persist the URL fragment when redirecting to the AMP version. This is needed because the server-generated amphtml
