...
window.ampCustomizeControls = function (api, $) {
  ...
  component.panelReady = function panelReady(panel) {
    ...
    const tooltipLink = tooltip.find('a'); // AMP panel triggers the input toggle for AMP preview.
    ...
    }(api.previewer.previewUrl.validate); // Listen for ampEnabled state changes.


    api.state('ampEnabled').bind(function (enabled) {
      checkbox.prop('checked', enabled);
      component.updatePreviewUrl(); // Preview tablet device when AMP is enabled.
    ...
    api.state('ampAvailable').bind(function (available) {
      checkbox.toggleClass('disabled', !available); // Show the unavailable tooltip if AMP is enabled.
    ...
    $('.devices-wrapper').prepend(ampToggleContainer); // Update tooltip for Customizer collapse button based on whether the pane is shown.
    ...
    const updateCollapseSidebarTooltip = () => {
      if (api.state('paneVisible').get()) {
        ...
        collapseSidebarButton.prop('title', (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Show Controls', 'amp'));
      }
    };

    updateCollapseSidebarTooltip();
    api.state('paneVisible').bind(updateCollapseSidebarTooltip); // User clicked link within tooltip, go to linked post in preview.
...
/******/ })()
;
//# sourceMappingURL=amp-customize-controls-legacy.js.map